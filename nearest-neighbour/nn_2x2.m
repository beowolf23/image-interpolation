    ...
    for i = 1 : n
        for j = 1 : n
            ...
            if(x_int(i) < 1.5 && y_int(j) < 1.5)
              nearest_neighbour_x = x1;
              nearest_neighbour_y = y1;
            elseif(x_int(i) < 1.5 && y_int(j) >= 1.5)
              nearest_neighbour_x = x1;
              nearest_neighbour_y = y2;
            elseif(x_int(i) >= 1.5 && y_int(j) < 1.5)
              nearest_neighbour_x = x2;
              nearest_neighbour_y = y1;
            elseif(x_int(i) >= 1.5 && y_int(j) >= 1.5)
              nearest_neighbour_x = x2;
              nearest_neighbour_y = y2;  
            ...
            X(i, j) = f(nearest_neighbour_x, nearest_neighbour_y);
        endfor
    endfor
    out = X;
endfunction